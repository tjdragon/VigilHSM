...
async function main() {
    ...
    if (!label || !base64Data) {
        console.error('Usage: npx ts-node src/sign.ts <label> <base64-data> [vault|softhsm]');
        process.exit(1);
    }
    ...
    try {
        ...
        console.log(`Signing data with key label: ${label} on ${hsmType}...`);
        ...
        const signature = await hsm.sign(label, payload);
